...
def meerkat_uvw(unix: float, direction: tuple[float], antpos: np.ndarray) -> np.ndarray:
    ...
    antpos = coordinates.EarthLocation(
        x=antpos[:, 0], y=antpos[:, 1], z=antpos[:, 2], unit="m"
    )
    ...
    antpos_gcrs = coordinates.GCRS(
        antpos.get_gcrs_posvel(datetime)[0],
        obstime=datetime,
        obsgeoloc=tel_p,
        obsgeovel=tel_v,
    )
    tel_gcrs = coordinates.GCRS(
        tel_p, obstime=datetime, obsgeoloc=tel_p, obsgeovel=tel_v
    )
