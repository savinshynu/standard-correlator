...
def create_uvh5(fob: IO[str], head_dict: tmeta, data_dict: tvis) -> None:
    ...
    uvh5_header.create_dataset(
        "longitude", data=head_dict["longitude"], dtype="d"
    )  # degrees
    uvh5_header.create_dataset(
        "latitude", data=head_dict["latitude"], dtype="d"
    )  # degrees
    ...
    uvh5_header.create_dataset(
        "Nants_telescope", data=head_dict["Nants_telescope"], dtype="i"
    )

    uvh5_header.create_dataset(
        "antenna_names",
        data=head_dict["antenna_names"],
        dtype=h5py.special_dtype(vlen=str),
    )
    uvh5_header.create_dataset(
        "antenna_numbers", data=head_dict["antenna_numbers"], dtype="i"
    )
    uvh5_header.create_dataset(
        "antenna_diameters", data=head_dict["antenna_diameters"], dtype="d"
    )
    uvh5_header.create_dataset(
        "antenna_positions", data=head_dict["antenna_positions"], dtype="d"
    )
    ...
    uvh5_header.create_dataset(
        "channel_width", data=head_dict["channel_width"], dtype="d"
    )
    ...
    uvh5_header.create_dataset(
        "polarization_array", data=head_dict["polarization_array"], dtype="i"
    )
    ...
    uvh5_header.create_dataset(
        "phase_center_ra", data=head_dict["phase_center_ra"], dtype="d"
    )
    uvh5_header.create_dataset(
        "phase_center_dec", data=head_dict["phase_center_dec"], dtype="d"
    )
    uvh5_header.create_dataset(
        "phase_center_epoch", data=head_dict["phase_center_epoch"], dtype="d"
    )
    uvh5_header.create_dataset(
        "phase_center_frame", data=head_dict["phase_center_frame"]
    )
    ...
    uvh5_header.create_dataset(
        "integration_time", data=head_dict["integration_time"], dtype="d"
    )
